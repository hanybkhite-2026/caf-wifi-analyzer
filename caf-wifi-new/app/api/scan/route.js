...
export async function GET() {
  // child_process is only available when running locally (not on Vercel cloud)
  try {
    ...
    return Response.json({
      networks: [],
      error: 'WiFi scanning unavailable here. Run locally (npm run dev) or as desktop app.',
      method: 'unavailable'
    });
  }
}
...
function parseMac(raw) {
  return raw.split('\n').slice(1).filter(l => l.trim()).map(line => {
    ...
    return { ssid: p[0]||'', bssid: p[1]||'',
      ...
      primaryCh: ch, freq: ch<=13 ? 2412+(ch-1)*5 : 5000+ch*5,
