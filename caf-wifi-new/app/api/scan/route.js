...
function parseLinux(raw) {
  return raw.split('\n').filter(l => l.trim() && !l.startsWith('IN-USE')).map(line => {
...
function parseMac(raw) {
  return raw.split('\n').slice(1).filter(l => l.trim()).map(line => {
    ...
    return { ssid: p[0] || '', bssid: p[1] || '',
      signal: p[2] ? parseInt(p[2]) : -70, primaryCh: ch,
      freq: ch <= 13 ? 2412 + (ch - 1) * 5 : 5000 + ch * 5,
      bw: 20, security: [p[6] || 'OPEN'], band: ch > 13 ? '5' : '2.4' };
  }).filter(n => n.ssid);
}
...
function parseWin(raw) {
  ...
  for (const b of raw.split(/\r?\nSSID \d+/)) {
    ...
    nets.push({ ssid, bssid, signal: sig ? Math.round(parseInt(sig) / 2 - 100) : -70,
      ...
      bw: 20, security: [sec], band: chN > 13 ? '5' : '2.4' });
  }
  return nets;
}
...
function doScan() {
  return new Promise(resolve => {
    ...
    if (platform === 'win32') {
      ...
      cmd = 'nmcli';
      args = ['-t', '-f', 'SSID,BSSID,CHAN,FREQ,SIGNAL,SECURITY',
              'dev', 'wifi', 'list', '--rescan', 'yes'];
    }
    execFile(cmd, args, { timeout: 12000 }, (err, stdout) => {
      if (err) return resolve({
        networks: [], error:
          'WiFi scanning unavailable in cloud. Run locally for real scanning.',
        method: 'none', platform });
      try {
        const nets = platform === 'linux' ? parseLinux(stdout)
          : platform === 'darwin' ? parseMac(stdout) : parseWin(stdout);
        const seen = new Set();
      ...
      } catch (e) { resolve({ networks: [], error: e.message, method: 'parse-error' }); }
    });
  });
}
