...
// ── STATIC DATA ──────────────────────────────────────────────────────────────
const APS_DATA = [
  { ssid:'CAF-WIFI-5G',  signal:-45, ch:36,  freq:'5170·5250 80MHz',  mac:'00:0B:86:12:34:56', vendor:'ARUBA NETWORKS',      quality:'Excellent', bw: 850 },
  { ssid:'CAF-WIFI-2G',  signal:-67, ch:6,   freq:'2417·2447 20MHz',  mac:'00:0B:86:78:90:AB', vendor:'ARUBA NETWORKS',      quality:'Good', bw: 150 },
  { ssid:'CAF-GUEST',    signal:-79, ch:52,  freq:'5735·5815 80MHz',  mac:'00:0B:86:CD:EF:01', vendor:'ARUBA NETWORKS',      quality:'Fair', bw: 300 },
  { ssid:'VTEL-Fiber',   signal:-85, ch:1,   freq:'2402·2422 20MHz',  mac:'7c:1c:f1:25:19:2c', vendor:'HUAWEI TECHNOLOGIES', quality:'Poor', bw: 50 },
  { ssid:'Mamon2_5G',    signal:-86, ch:128, freq:'5170·5250 80MHz',  mac:'98:da:c4:26:21:87', vendor:'TP-LINK TECHNOLOGIES',quality:'Poor', bw: 600 },
  { ssid:'*hidden*',     signal:-87, ch:6,   freq:'5170·5250 80MHz',  mac:'9e:da:c4:26:21:87', vendor:'GENERIC VENDOR',      quality:'Poor', bw: 100 },
];
...
export default function App() {
  ...
  useEffect(() => {
    if (locating) {
      const target = APS_DATA.find(ap => ap.ssid === locating);
      if (target) {
        setCurrentSignal(target.signal);
  ...
  return (
    <div style={s.app}>
      ...
      {locating && (
        <div style={s.overlay}>
          <div style={{...s.modal, textAlign:'center', padding:'40px'}}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'32px'}}>
              <div style={{textAlign:'left'}}>
                <div style={{fontSize:'11px', color:T.blue, fontWeight:'800', letterSpacing:'0.1em'}}>LOCATING INFRASTRUCTURE</div>
                <div style={{fontSize:'20px', fontWeight:'800'}}>{locating}</div>
              </div>
              <button style={{background:'transparent', border:'none', color:T.red, fontSize:'24px', cursor:'pointer'}} onClick={()=>setLocating(null)}>✕</button>
            </div>
            
            <div style={{fontSize:'96px', fontWeight:'900', color:T.blue, textShadow:`0 0 40px ${T.blue}55`}}>
              {Math.round(currentSignal)}<span style={{fontSize:'24px', marginLeft:'4px', color:T.muted}}>dBm</span>
            </div>
            
            <div style={{marginTop:'40px'}}>
              <div style={{height:'12px', background:T.border, borderRadius:'6px', overflow:'hidden'}}>
                <div style={{width:`${Math.min(100, Math.max(0, (currentSignal + 100) * 1.5))}%`, height:'100%', background:T.blue, transition:'width 0.1s'}} />
              </div>
              <div style={{display:'flex', justifyContent:'space-between', marginTop:'12px'}}>
                <div style={{fontSize:'10px', fontWeight:'700', color:T.muted}}>POOR</div>
                <div style={{fontSize:'10px', fontWeight:'700', color:T.green}}>EXCELLENT</div>
              </div>
            </div>

            <div style={{marginTop:'40px', background:T.card2, padding:'20px', borderRadius:'12px', border:`1px solid ${T.border}`}}>
              <div style={{fontSize:'12px', fontWeight:'700', color:T.blue, marginBottom:'8px'}}>GEIGER AUDIO FEEDBACK</div>
              <p style={{fontSize:'11px', color:T.muted, lineHeight:'1.5'}}>Beep rate and pitch increase as you approach the Access Point hardware.</p>
              <button 
                style={{marginTop:'16px', background:isMuted?T.border:T.blue, color:'#fff', border:'none', padding:'8px 16px', borderRadius:'8px', cursor:'pointer', fontSize:'11px', fontWeight:'800'}}
